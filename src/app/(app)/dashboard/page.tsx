...
const DOCUMENTS = [
  ...
  { id: 2, name: "PAN Card", category: "Identity", emoji: "🪪", color: "bg-purple-500/20 text-purple-400", tags: ["IPFS","AES-256","On-chain"], cid: "Qm9aL...c4Wz", date: "12 Jan 2025", size: "0.8 MB", onChain: true, uploaded: true },
  { id: 3, name: "Passport", category: "Identity", emoji: "📕", color: "bg-purple-500/20 text-purple-400", tags: ["✓ Verified","IPFS","AES-256","On-chain"], cid: "Qm2bM...d5Xz", date: "15 Jan 2025", size: "2.1 MB", onChain: true, uploaded: false },
  ...
  { id: 5, name: "B.Tech Degree", category: "Education", emoji: "🎓", color: "bg-amber-500/20 text-amber-400", tags: ["✓ Verified","IPFS","AES-256","On-chain"], cid: "Qm4dO...f7Wz", date: "22 Jan 2025", size: "3.4 MB", onChain: true, uploaded: true },
    ...
    <div>
      ...
      {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <StatCard icon="📄" label="Total Documents" value={DOCUMENTS.length.toString()} sub="AES-256 Encrypted" />
          <StatCard icon="⛓" label="On-chain Proofs" value={DOCUMENTS.filter(d => d.onChain).length.toString()} sub="Ethereum Mainnet" />
          <StatCard icon="💾" label="Vault Size" value={(DOCUMENTS.reduce((sum, d) => sum + parseFloat(d.size), 0)).toFixed(1) + " MB"} sub="89% storage free" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(doc => (
          <div 
            key={doc.id} 
            onClick={() => !doc.uploaded && router.push('/documents')}
            className={cn(
              "group bg-vault-surface border rounded-xl p-[18px] flex flex-col gap-3 transition-all duration-200",
              doc.uploaded 
                ? "border-white/[0.07] hover:border-vault-gold/25 hover:-translate-y-0.5 cursor-pointer" 
                : "border-white/[0.04] bg-vault-surface/50 opacity-50 cursor-pointer hover:opacity-60"
            )}
          >
            {/* Icon + Menu */}
            <div className="flex items-start justify-between">
              <div className="relative w-8 h-8">
                <div className={cn("absolute inset-0 rounded-xl blur-sm", doc.uploaded ? "bg-vault-gold/20" : "bg-gray-500/10")} />
                <div className={cn("relative w-8 h-8 rounded-xl flex items-center justify-center", 
                  doc.uploaded 
                    ? "bg-gradient-to-br from-vault-gold to-vault-goldLight" 
              ...
              {!doc.uploaded && (
                <span className="text-xs font-mono text-muted-foreground bg-white/[0.05] px-2 py-1 rounded">
                  Not uploaded
                </span>
              )}
            </div>
            ...
            <div>
              ...
              <p className="font-mono text-[11px] text-muted-foreground">{doc.uploaded ? `${doc.date} · ${doc.size}` : "Click to upload"}</p>
            </div>
